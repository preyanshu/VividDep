...
const data=[
  ...
  return (<>
    <div className='mainbg'>
      ...
      {
        flag3 &&<i class="fa-solid fa-arrow-left fa-xl ml-5 mx-3 " style={{position:"absolute",right:"17px",marginLeft:"200px",top:"37px"}} onClick={()=>{
          if(flag2==false){
            setflag2(true);
          }
          else{
...
<div className="a shadow " style={{height:79+"vh",width:78+"vw",backgroundColor:"white",borderRadius:13+"px",border:"0px solid black",padding:25+"px",marginTop:30+"px"}}>
  {
    ...
    {!flag && <><div style={{display:"flex",justifyContent:"end",alignItems:"end"}}>
      {
        !filter &&  <div className="btn btn-success me-3" style={{height:"40px",width:"160px"}}  onClick={()=>{
          if(filter){
            setfilter(false);
          }
          else{
        ...
        }}><b><i class="fa-solid fa-sort fa-xl me-2"></i>Apply Filter </b></div>
      }
      {
        ...
        }}  ><b><i class="fa-solid fa-sort fa-xl me-2"></i> Remove Filter </b></div>
      }
  ...
  <option value="8">8</option>
    ...
    <div  className="m-3 p-5"style={{border:"0px solid black",height:"90%",display:"flex",flexDirection:"column",alignItems:"center",overflowY:"auto"}}>
      {!filter &&
      details
       .map((e, index) => (
